...
class LearnerImportPage {
    ...
    static async animateProgress(fill, status, done) {
        ...
        for (const pct of steps) {
            ...
            status.textContent = pct < 100 ? `Uploading… ${pct}%` : "Scanning for duplicates and errors…";
        }
    ...
    static async runPreview() {
        ...
        try {
            ...
            document.getElementById("importUploadSection")?.classList.add("st-hidden");

            document.getElementById("importPreviewSection")?.classList.remove("st-hidden");
    ...
    static statusBadge(status) {
        ...
        const map = {
            valid: '<span class="st-import-status-badge st-import-status-badge--valid">Valid</span>',
            duplicate: '<span class="st-import-status-badge st-import-status-badge--duplicate">Duplicate</span>',
            error: '<span class="st-import-status-badge st-import-status-badge--error">Error</span>'
        };
    ...
    static bindPreviewActions() {
        ...
        document.querySelector("[data-preview-cancel]")?.addEventListener("click", () => {

            document.getElementById("importPreviewSection")?.classList.add("st-hidden");

            document.getElementById("importUploadSection")?.classList.remove("st-hidden");

            this.resetUploadState();

        });

        document.querySelector("[data-preview-confirm]")?.addEventListener("click", async () => {
            ...
            try {
                ...
                Toast?.success(`${this.preview.valid} learner(s) imported successfully.`);
    ...
    static bindSuccessActions() {
        ...
        document.querySelector("[data-import-another]")?.addEventListener("click", () => {
            ...
            document.getElementById("importSuccessSection")?.classList.add("st-hidden");
...
(function bootLearnerImport() {
    let started = false;
    const start = () => { if (!started) { started = true; LearnerImportPage.init(); } };
