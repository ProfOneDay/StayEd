...
class LearnerRecordsHub {
    ...
    static state = {
        ...
        perPage: 6,
        selected: { modular: new Set(), "face-to-face": new Set(), blended: new Set() }
    };
    ...
    static bindFilters() {
        ...
        document.querySelector("[data-records-search]")?.addEventListener("input", e => {
            this.state.search = e.target.value.toLowerCase();
            this.resetPages();
            this.renderActiveTab();
        });

        document.querySelector("[data-records-filter-clc]")?.addEventListener("change", e => {
            this.state.clc = e.target.value;
            this.resetPages();
            this.renderActiveTab();
        });

        document.querySelector("[data-records-filter-level]")?.addEventListener("change", e => {
            this.state.level = e.target.value;
            this.resetPages();
            this.renderActiveTab();
        });

        document.querySelector("[data-records-filter-risk]")?.addEventListener("change", e => {
    ...
    static bindSaveExport() {
        ...
        document.querySelector("[data-records-save]")?.addEventListener("click", () => {
            Toast?.success("Changes saved.");
        });

        document.querySelector("[data-records-export]")?.addEventListener("click", () => {
            Toast?.info("Export will be available once connected to the server.");
        });

        document.querySelector("[data-modular-bulk-export]")?.addEventListener("click", () => {
            Toast?.info(`Exporting ${this.state.selected.modular.size} learner record(s)…`);
        });

        document.querySelector("[data-modular-bulk-contact]")?.addEventListener("click", () => {
            Toast?.success(`Marked ${this.state.selected.modular.size} learner(s) as Contacted.`);
        });

        document.querySelector("[data-f2f-bulk-export]")?.addEventListener("click", () => {
            Toast?.info(`Exporting ${this.state.selected["face-to-face"].size} learner record(s)…`);
        });

        document.querySelector("[data-blended-bulk-export]")?.addEventListener("click", () => {
            Toast?.info(`Exporting ${this.state.selected.blended.size} learner record(s)…`);
        });
    ...
    static modularRow(l) {
        ...
        const barClass = l.risk === "High" ? "st-progress-fill--high"
            : l.risk === "Moderate" ? "st-progress-fill--moderate" : "st-progress-fill--low";

        const labelClass = l.risk === "High" ? "st-progress-cell-label--danger" : "";
    ...
    static renderBlendedSummary(rows) {
        ...
        const avgModules = Math.round(
            rows.reduce((sum, l) => sum + ((l.modules?.completed / l.modules?.total) || 0), 0) / rows.length * 100
        );

        const avgAttendance = Math.round(
            rows.reduce((sum, l) => sum + (l.attendance_rate || 0), 0) / rows.length * 100
        );

        const atRisk = rows.filter(l => l.risk === "High" || l.risk === "Moderate").length;

        this.set("[data-blended-modules]", `${avgModules}%`);
        this.set("[data-blended-attendance]", `${avgAttendance}%`);
        this.set("[data-blended-completion]", `${Math.round((avgModules + avgAttendance) / 2)}%`);
    ...
    static renderTable({ rows, bodySelector, infoSelector, pagesSelector, pageKey, colspan, rowRenderer }) {
        ...
        this.set(infoSelector, `Showing ${infoStart}\u2013${infoEnd} of ${total} learners`);
    ...
    static bindRowInteractions(container) {
        ...
        container.querySelectorAll("[data-contact-status], [data-attendance-status]").forEach(select => {
    ...
    static SELECTION_CONFIG = {
        ...
        "face-to-face": { attr: "data-row-select-f2f", selectAll: "[data-f2f-select-all]", bar: "[data-f2f-bulk-bar]", count: "[data-f2f-bulk-count]" },
        blended: { attr: "data-row-select-blended", selectAll: "[data-blended-select-all]", bar: "[data-blended-bulk-bar]", count: "[data-blended-bulk-count]" }
    };
    ...
    static bindSelectionCheckboxes(container) {
        ...
        const attrCamel = config.attr.replace(/-([a-z])/g, (_, c) => c.toUpperCase()).replace("data", "");
...
(function bootRecordsHub() {
    let started = false;
    const start = () => { if (!started) { started = true; LearnerRecordsHub.init(); } };
